...
- 在 WebSocket中，浏览器和服务器只需要完成一次握手，就可以创建持久性的连接（即长连接），并进行双向数据传输。
...
首先HTTP有 `1.1` 和 `1.0` 之说，也就是所谓的 `keep-alive` ，把多个HTTP请求合并为一个，但是 `Websocket` 其实是一个新协议，跟HTTP协议基本没有关系，只是为了兼容现有浏览器的握手规范而已，也就是说它是HTTP协议上的一种补充，与HTTP有交集但不是全部
...
参考自：https://www.zhihu.com/question/20215561
