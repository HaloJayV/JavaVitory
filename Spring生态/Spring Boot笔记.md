...
以后我们导入依赖默认是不需要写版本,  他来真正管理Spring Boot应用里面的所有依赖版本；；（没有在dependencies里面管理的依赖自然需要声明版本号）
...
​		Spring的底层注解@Import，给容器中导入一个组件；导入组件AutoConfigurationPackages.Registrar.class；
